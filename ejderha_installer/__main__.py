...
def createApp (connect):
    appname = "ejderha" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def botlog (String, Api, Hash):
    ...
    Photo = await Client.upload_file(file='alphalogo.jpg')
...
if __name__ == "__main__":
    ...
    if os.path.isdir("./EjderhaUserBot/"):
        rm_r("./EjderhaUserBot/")
    repo = Repo.clone_from(str1,"./EjderhaUserBot/", branch="main")
    ...
    config['DEFAULT_BIO'] = "@EjderhaUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/HerLockDev/EjderhaUserBot"
    config['WARN_LIMIT'] = "3"
    config['WARN_MODE'] = "gmute"
    config['LANGUAGE'] = LANGUAGE
    config['TELEGRAPH_SHORT_NAME'] = "alphauserbot"
    config["TMP_DOWNLOAD_DIRECTORY"] = "./DOWNLOADS/"
